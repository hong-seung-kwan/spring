...
public interface BookRepository extends JpaRepository<Book, Integer> {
	...
	// SELECT * FROM tbl_book WHERE title = '자바프로그래밍입문'; // 1.sql 그대로 작성
	...
	// 책의 가격이 30000원이상이고 출판사가 남가람북스인 데이터 검색
	// SELECT * FROM tbl_book WHERE price > 30000 AND publisher = '남가람북스'
	@Query(value = "select * from tbl_book where :price > 30000 AND publisher = :publisher", nativeQuery = true)
